...
test("卡片不含 tag:action 容器（schema 2.0 已移除该能力，200861）", () => {
  const cards = [welcomeCard("测试"), commandPanelCard(), statusCard("connected", [])];
  let actionCount = 0;
...
test("按钮直接平铺在 body.elements 且带 behaviors callback + width fill", () => {
  ...
  for (const b of buttons) {
    ...
    assert.ok(behaviors[0]?.type === "callback", "按钮用 behaviors callback 回传");
...
test("按钮 op 路由到期望的命令", () => {
  ...
  const ops = buttons.map((b) => (b.behaviors as Array<{ value: Json }>)[0]!.value.op as string);
...
test("button helper: primary/danger 样式", () => {
  const primary = button("批准", { op: "approve", approvalId: "a1" }, "primary") as Json;
  assert.equal(primary.type, "primary");
  const danger = button("拒绝", { op: "deny", approvalId: "a1" }, "danger") as Json;
