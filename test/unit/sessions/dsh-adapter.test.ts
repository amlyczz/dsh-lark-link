...
function fakeRegistry(opts: { onEvent?: (agent: unknown, ev: unknown) => void } = {}) {
  ...
  const registry = {
    ...
    async create(createOpts: { sessionId: string }) {
      ...
      agentNo.n++;
      return { agent, dispose: async () => agents.delete(createOpts.sessionId) };
    },
    get(id: string) {
...
const ctxOf = (registry: ReturnType<typeof fakeRegistry>, agentDefaultModel?: unknown) =>
  ({ agents: registry, agentDefaultModel }) as unknown as Parameters<typeof createDshAdapter>[0]["ctx"];
...
test("adapter: agents.create receives agentOptions + installModelSelection setup from agentDefaultModel", async () => {
  ...
  assert.equal(typeof opts.setup, "function", "setup must be provided to wire installModelSelection");
  assert.ok(handle.agentId.includes("lark-link:dm:ou_user_1"), "session prefix applied");
});
...
test("adapter: create works without agentDefaultModel (no provider/model)", async () => {
  ...
  assert.equal(registry.created.length, 1);
  const opts = registry.created[0] as { agentOptions?: unknown; setup?: unknown };
  assert.equal(opts.agentOptions, undefined, "no agentOptions without default model");
...
test("adapter: toSessionEventOut maps turn/start to a bridge event", async () => {
  ...
  agent.ctx.emit("session/event", { type: "turn/start", seq: 0, time: Date.now(), data: {} });

  assert.ok(events.some((e) => e.type === "turn/start"), "turn/start forwarded to bridge listeners");
});
