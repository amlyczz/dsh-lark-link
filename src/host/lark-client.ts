...
export function parseCredentials(raw: string | undefined): LarkCredentials | undefined {
...
export async function clearCredentials(store: CredentialsStore, ref: string): Promise<void> {
...
export interface SdkClient {
  ...
  im: {
    ...
    messageReaction: { create(opts: Record<string, unknown>): Promise<unknown> };
 ...
 */
export async function buildLarkClient(opts: BuildLarkClientOptions): Promise<FeishuClientLike> {
  ...
  const client: FeishuClientLike = {
    ...
    async getBotInfo() {
      ...
      const bot = (res?.bot as Record<string, unknown> | undefined) ??
        ((res?.data as Record<string, unknown> | undefined)?.bot as Record<string, unknown> | undefined);
      const openId = (bot?.open_id as string | undefined) ?? (res?.data as Record<string, unknown> | undefined)?.open_id as string | undefined;
    ...
    async sendMessage(params) {
      ...
      const p = params as { receive_id_type: string; params: Record<string, unknown> };
    ...
    async listMessages(params) {
      ...
      const items = (res?.items as Array<Record<string, unknown>> | undefined) ??
        ((res?.data as Record<string, unknown> | undefined)?.items as Array<Record<string, unknown>> | undefined) ??
        [];
      return {
        items: items.map((i) => ({
    ...
    async uploadFile(params) {
      const p = params as { file_type: string; file_name?: string; file: Buffer };
