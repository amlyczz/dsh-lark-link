...
import type { ChatMode, ChatType, FeishuInboundMessage, FeishuMsgType } from "../common/types.ts";
...
function pickText(contentRaw: string | undefined, msgType: FeishuMsgType): string | undefined {
  ...
  try {
    ...
    const content = parsed.content as { paragraphs?: Array<{ elements?: Array<{ text_run?: { content?: string } }> }> } | undefined;
    if (msgType === "post" && content?.paragraphs) {
      return content.paragraphs
        .map((p) => (p.elements ?? []).map((e) => e.text_run?.content ?? "").join(""))
        .join("\n");
    }
 ...
 */
export function normalizeInbound(raw: RawEvent, opts: NormalizeOptions = {}): FeishuInboundMessage | undefined {
  ...
  const senderOpenId =
    raw.sender?.sender_id?.open_id ?? raw.operator?.operator_id?.open_id ?? "unknown";

  const mentions = (msg.mentions ?? []).map((m) => m.id?.open_id ?? m.id?.user_id ?? m.name ?? "").filter(Boolean);
  const mentionedBot =
    opts.mentionedBot ??
    (opts.botOpenId !== undefined ? mentions.includes(opts.botOpenId) : mentions.length > 0);
...
export function isBotMentioned(text: string | undefined, botName = ""): boolean {
...
export interface FeishuClientLike {
  ...
  listMessages?(params: unknown): Promise<{ items?: Array<{ message_id?: string; create_time?: string }> }>;
}
...
export interface TransportDeps {
  ...
  normalize?: typeof normalizeInbound;  logger?: { info(msg: string): void; warn(msg: string): void; error(msg: string): void };
}
