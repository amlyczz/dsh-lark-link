...
export function createConnectionSupervisor(
	...
	async function ensureConnected(): Promise<void> {
		...
		if (deps.quota.tripped()) {
			setState(
				"quarantined",
				`quota breaker tripped (${deps.cfg.quotaLimit}/${deps.cfg.quotaWindowMinutes}min); retry after reset`,
			);
			return;
		}
		if (reconnectAttempts >= deps.cfg.maxReconnectAttempts) {
			...
			setState(
				"quarantined",
				`reconnect attempts exhausted (${reconnectAttempts}); circuit breaker armed`,
			);
			return;
		}
		setState("connecting");
		...
		if (deps.transport.isConnected()) {
			...
			if (deps.quota.tripped()) {
				setState(
					"quarantined",
					`quota breaker tripped after ${reconnectAttempts} failed connects`,
				);
				return;
			}
			setState(
				"reconnecting",
				`connect failed (attempt ${reconnectAttempts}/${deps.cfg.maxReconnectAttempts})`,
			);
		}
	}
