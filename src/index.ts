...
import { createTransport, extractUploadKey, type FeishuClientLike } from "./inbound/transport.ts";
...
import { createBridgeContext, type FeishuSender } from "./application/bridge-context.ts";
import { createMessageHandler } from "./application/message-handler.ts";
import { createCommandRouter, type DshCommandRegistry } from "./application/command-router.ts";
...
import { formatStatusLine, statusDetailLines } from "./application/status-formatter.ts";
...
export function apply(ctx: Context, rawConfig: unknown): void {
  ...
  const sender: FeishuSender = {
    ...
    async sendText(chatId, text) {
      ...
      await client.sendMessage({
        ...
        params: { receive_id: chatId, msg_type: "text", content: JSON.stringify({ text }) },
      });
    },
    async sendCard(chatId, card) {
      ...
      await client.sendMessage({
        receive_id_type: chatId.startsWith("oc_") ? "chat_id" : "open_id",
        params: { receive_id: chatId, msg_type: "interactive", content: JSON.stringify(card) },
      });
    },
    async addReaction(messageId, emojiType) {
      const client = getLarkClient();
      if (!client?.addReaction) throw new Error("lark client not ready");
      await client.addReaction({ message_id: messageId, emoji_type: emojiType });
    },
    async sendFile(chatId, fileKey, type) {
      ...
      await client.sendMessage({
        receive_id_type: chatId.startsWith("oc_") ? "chat_id" : "open_id",
        params: { receive_id: chatId, msg_type: type, content: JSON.stringify(type === "image" ? { image_key: fileKey } : { file_key: fileKey }) },
      });
    },
    async listMessages({ chatId, startTimeMs, endTimeMs }) {
      const client = getLarkClient();
      if (!client?.listMessages) return [];
      const res = await client.listMessages({ container_id_type: "chat", container_id: chatId, start_time: String(startTimeMs), end_time: String(endTimeMs) });
      return (res.items ?? []).map((i) => ({ messageId: i.message_id ?? "", timestampMs: Number(i.create_time ?? 0) }));
    },
  };
    ...
    async deliver(env, payload) {
      ...
      try {
        if (payload.kind === "text") {
          if (payload.card !== undefined) await sender.sendCard(chatId, payload.card);
  ...
  const groupTrigger = createGroupTrigger({
    cfg: () => ({ policy: getCfg().groupPolicy, keywords: getCfg().groupKeywords, alsoOnReply: getCfg().alsoOnReply }),
  ...
  const dshCommands: DshCommandRegistry = {
    has: (name) => (ctx as unknown as { commands?: { has?(n: string): boolean } }).commands?.has?.(name) ?? false,
    async run(name, rawInput, agentId) {
      try {
        const registry = (ctx as unknown as { commands?: { run?(n: string, r: string, a: string): Promise<{ kind: string; text?: string }> } }).commands;
        if (!registry?.run) return { kind: "error", text: "commands service unavailable" };
  ...
  const bridgeHandler = async (name: string, _rawInput: string, msg: FeishuInboundMessage): Promise<boolean> => {
    switch (name) {
      case "status":
      case "feishu-config":
      case "lark-config":
        await sender.replyTo(msg, formatStatusLine(status.get()) + "\n\n" + statusDetailLines(status.get()).join("\n"));
        return true;
      case "support":
      case "doctor":
      ...
      case "sessions": {
        ...
        const lines = keys.length
          ? keys.map((k) => `- ${k}`)
          : ["（无活跃会话）"];
        await sender.replyTo(msg, `**会话列表 (${keys.length})**\n\n` + lines.join("\n"));
        return true;
      }
      case "help":
  ...
  const commandRouter = createCommandRouter({ ctx: bridge, commands: dshCommands, bridgeHandler });
  ...
  const conversations = createConversationManager({
    ...
    onEvent: (key, event) => {
      void forwarder.onSessionEvent(key, event).catch((e) => logger.warn(`forwarder: ${String(e)}`));
  ...
  const startBridge = async (): Promise<void> => {
    ...
    status.refreshCounters({ outboxPending: outbox.pendingCount(), outboxFailed: outbox.failedCount() });
  ...
  ctx.tools.register(
    defineTool({
      ...
      parameters: {
        path: { type: "string", required: true, description: "Absolute local path" },
      ...
      async execute(args, exec) {
        ...
        const key = sessionId.startsWith(prefix) ? sessionId.slice(prefix.length) : sessionId;
  ...
  const commandsCtx = ctx as unknown as { commands?: { register(d: unknown): void } };
  const registerCmd = (name: string, description: string, handler: () => Promise<string>): void => {
    commandsCtx.commands?.register?.({ name, description, handler: async () => ({ kind: "success", text: await handler() }) });
  };
  ...
  try {
    (ctx as unknown as { systemPrompt?: { section(s: unknown): void } }).systemPrompt?.section?.({
      priority: 200,
      section: () => ({
        role: "system",
        content: [
  ...
  ctx.effect(() => {
    ...
    const sweep = setInterval(() => {
      ...
      if (n > 0) status.refreshCounters({ outboxPending: outbox.pendingCount(), outboxFailed: outbox.failedCount() });
    }, 60_000);
    sweep.unref?.();
    return async () => {
      clearInterval(sweep);
