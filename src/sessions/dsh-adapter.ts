...
import type { SessionEventOut, DshSessionBackend, AgentHandle, AttachmentInput } from "./dsh-session-backend.ts";
...
function toSessionEventOut(ev: SessionEvent): SessionEventOut | undefined {
  switch (ev.type) {
    ...
    case "tool/result":
      return { type: "tool/result", name: ev.data.message.content?.[0]?.type ?? "?", error: ev.data.error };
    default:
      return undefined;
  }
}
...
export function createDshAdapter(deps: DshAdapterDeps): DshSessionBackend {
  ...
  async function ensureAgent(key: string): Promise<AgentHandle> {
    ...
    const handle: AgentHandle = {
      ...
      onEvent(fn) {
        const set = listeners.get(key) ?? new Set<(e: SessionEventOut) => void>();
        set.add(fn);
    ...
    const disp = agent.ctx.on("session/event", (_session: unknown, ev: SessionEvent) => {
